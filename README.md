...
환경설정은 하단 왼쪽 제목줄의 `환경설정` 버튼으로 별도 창에서 엽니다. 배경·글자 크기·색상·투명도와 함께 Windows에 설치된 글꼴을 선택할 수 있습니다.

## 상단에 다른 프로그램 넣기

- 상단의 `프로그램 연결`을 누르면 현재 실행 중인 일반 프로그램 창을 선택해 인터넷 영역 안에 넣을 수 있습니다.
- 하단 왼쪽 `환경설정`에서 실행 파일을 등록하면 북마크 줄의 프로그램 버튼으로 바로 실행하고 연결할 수 있습니다.
- 연결된 프로그램이 최대화나 전체화면을 요청해도 상단 영역의 크기와 창 형식을 다시 적용합니다.
- 실행하거나 연결한 프로그램은 인터넷 탭 옆에 별도 탭으로 표시됩니다. 프로그램 탭과 인터넷 탭을 오갈 수 있고, 프로그램 탭의 `×`로 해당 프로그램에 정상 종료를 요청할 수 있습니다.
- `인터넷으로 돌아가기`를 누르면 연결한 프로그램은 원래 독립 창으로 돌아갑니다.
- 일반 창 모드 프로그램을 대상으로 합니다. 독점 전체화면을 사용하는 게임이나 보안 프로그램은 Windows 정책상 연결되지 않을 수 있습니다.
